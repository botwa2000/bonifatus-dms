...
class GoogleDriveClient:
    ...
    async def _get_authenticated_service(self):
        ...
        try:
            ...
            logger.error(
                f"Failed to authenticate Google Drive for user {self.user_id}: {e}"
            )
            return None
    ...
    async def check_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not service:
                return {"connected": False, "error": "No authentication available"}
            ...
            return {"connected": False, "error": f"API Error: {e.resp.status}"}
        except Exception as e:
            logger.error(
                f"Google Drive connection check failed for user {self.user_id}: {e}"
            )
    ...
    async def initialize_user_folder(self) -> bool:
        ...
        try:
            ...
            if self._user and self._user.google_drive_folder_id:
                try:
                    service.files().get(
                        fileId=self._user.google_drive_folder_id
                    ).execute()
                    logger.info(
                        f"Bonifatus DMS folder already exists for user {self.user_id}"
                    )
                    return True
                except HttpError as e:
                    if e.resp.status == 404:
                        logger.warning(
                            f"Stored folder ID invalid for user {self.user_id}, creating new"
                        )
                    else:
                        raise
            ...
            main_folder = (
                service.files().create(body=main_folder_metadata, fields="id").execute()
            )
            ...
            for category_name in default_categories:
                ...
                try:
                    service.files().create(
                        body=category_folder_metadata, fields="id"
                    ).execute()
                    logger.info(
                        f"Created category folder '{category_name}' for user {self.user_id}"
                    )
                except HttpError as e:
                    logger.warning(
                        f"Failed to create category folder '{category_name}': {e}"
                    )
            ...
            logger.error(
                f"Failed to initialize Google Drive folder for user {self.user_id}: {e}"
            )
            return False
    ...
    async def upload_file(
        ...
        try:
            ...
            if file_result:
                logger.info(
                    f"Successfully uploaded file {filename} for user {self.user_id}"
                )
                return {
    ...
    async def get_download_url(self, file_id: str) -> Optional[str]:
        ...
        try:
            ...
            file_metadata = (
                service.files()
                .get(fileId=file_id, fields="webContentLink,webViewLink")
                .execute()
            )

            # Return direct download link if available
            return file_metadata.get("webContentLink") or file_metadata.get(
                "webViewLink"
            )
    ...
    async def delete_file(self, file_id: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Deleted file {file_id} from Google Drive for user {self.user_id}"
            )
            return True
    ...
    async def list_files(
        self, folder_id: Optional[str] = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """
            ...
            return [
                {
                    ...
                    "size": (
                        int(file_item.get("size", 0)) if file_item.get("size") else 0
                    ),
                    ...
                    processed_changes.append(
                        {
                            ...
                            "change_type": (
                                "deleted" if file_info.get("trashed") else "modified"
                            ),
                        }
                    )
    ...
    async def _get_or_create_category_folder(
        self, service, category_name: str, parent_folder_id: str
    ) -> Optional[str]:
        """
