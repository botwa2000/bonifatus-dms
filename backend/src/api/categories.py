...
async def list_categories(
    ...
    language: str = Query(
        "en", regex="^(en|de)$", description="Language for category names"
    ),
...
async def create_category(
    ...
    try:
        ...
        user_categories_count = (
            db.query(Category)
            .filter(Category.user_id == user.id, Category.is_system_category == False)
            .count()
        )
...
async def get_category(
    ...
    try:
        ...
        recent_documents = (
            db.query(Document)
            .filter(Document.category_id == category.id, Document.user_id == user.id)
...
async def delete_category(
    category_id: int,
    move_documents_to: Optional[int] = Query(
        None, description="Category ID to move documents to"
    ),
    credentials: HTTPAuthorizationCredentials = Depends(security),
    db: Session = Depends(get_db),
):
    ...
    try:
        ...
        document_count = (
            db.query(Document)
            .filter(Document.category_id == category_id, Document.user_id == user.id)
            .count()
        )
        ...
        if document_count > 0:
            if move_documents_to:
                ...
                db.query(Document).filter(
                    Document.category_id == category_id, Document.user_id == user.id
...
async def list_category_documents(
    ...
    sort_by: str = Query(
        "created_at", regex="^(created_at|updated_at|filename|view_count)$"
    ),
            ...
            "documents": [
                {
                    ...
                    "created_at": doc.created_at.isoformat(),
                    "last_viewed_at": (
                        doc.last_viewed_at.isoformat() if doc.last_viewed_at else None
                    ),
                }
...
async def suggest_category(
    ...
    try:
        ...
        suggestions = await category_service.suggest_categories(
            user_id=user.id, text=text, filename=filename, language=language
        )
