...
from src.database.models import User, UserSettings, Document, Category, AuditLog, UserTier
...
class UserService:
    ...
    async def get_usage_statistics(self, user_id: int, period: str = "month") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            storage_query = self.db.query(
                func.coalesce(func.sum(Document.file_size_bytes), 0).label('total_storage')
            ...
            ).group_by(Category.id, Category.name_en, Category.color).all()
            ...
            popular_docs = self.db.query(Document).filter(
                Document.user_id == user_id
            ).order_by(desc(Document.view_count)).limit(5).all()
    ...
    async def delete_user_account(
        self,
        user_id: int,
        delete_google_drive_files: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if delete_google_drive_files:
                try:
                    ...
                    logger.warning(f"Google Drive deletion failed for user {user_id}: {drive_error}")
            
            # Delete database records (cascade will handle related data)
            self.db.delete(user)
            self.db.commit()
            
            logger.info(f"Deleted user account {user_id} with {document_count} documents")
    ...
    def _get_trial_info(self, user: User) -> Dict[str, Any]:
        ...
        now = datetime.utcnow()
        is_trial_active = (
            user.tier == UserTier.PREMIUM_TRIAL and
            user.trial_ended_at and
            now < user.trial_ended_at
        )
        ...
        return {
            ...
            "trial_ends": user.trial_ended_at.isoformat() if user.trial_ended_at else None,
        ...
        try:
            ...
            if data.get("documents"):
                doc_writer = csv.DictWriter(
                    output,
                    fieldnames=data["documents"][0].keys()
                )
            ...
            if data.get("categories"):
                cat_writer = csv.DictWriter(
                    output,
                    fieldnames=data["categories"][0].keys()
                )
