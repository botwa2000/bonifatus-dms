...
class SearchService:
    ...
    async def advanced_search(
        ...
        try:
            ...
            formatted_documents = await self._format_search_results(documents, query, highlight=True)
    ...
    async def get_search_suggestions(
        ...
        try:
            ...
            category_matches = self.db.query(Category).filter(
                and_(
                    or_(
        ...
        for term in search_terms:
            if len(term) >= 2:
                ...
                term_conditions = or_(
                    ...
                    text("EXISTS (SELECT 1 FROM unnest(extracted_keywords) AS keyword WHERE keyword ILIKE :term)").params(term=f"%{term}%"),
                    text("EXISTS (SELECT 1 FROM unnest(user_keywords) AS keyword WHERE keyword ILIKE :term)").params(term=f"%{term}%")
                )
    ...
    def _apply_advanced_filters(self, query, filters: Dict[str, Any]):
        ...
        if filters.get("file_types"):
            extensions = [f".{ft}" if not ft.startswith(".") else ft for ft in filters["file_types"]]
    ...
    async def _format_search_results(
        ...
        for document in documents:
            ...
            if highlight and query:
                result["highlighted_title"] = self._highlight_text(document.title or "", query)
                result["highlighted_description"] = self._highlight_text(document.description or "", query)
