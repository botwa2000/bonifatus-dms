...
class CategoryService:
    ...
    async def get_user_categories(self, user_id: int, include_system: bool = True, 
                                include_user: bool = True, language: str = "en") -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if include_user:
                conditions.append(and_(Category.user_id == user_id, Category.is_system_category == False))
    ...
    async def create_user_category(self, user_id: int, category_data: Dict[str, Any]) -> Optional[Category]:
        """Create user-defined category"""
        try:
            # Check user's category limit
            user_categories_count = self.db.query(Category).filter(
                and_(Category.user_id == user_id, Category.is_system_category == False)
            ).count()
    ...
    async def update_category(self, category_id: int, user_id: int, updates: Dict[str, Any]) -> Optional[Category]:
        ...
        try:
            ...
            # Apply updates
            allowed_fields = ["name_en", "name_de", "description_en", "description_de", "color", "icon", "keywords"]
    ...
    async def delete_category(self, category_id: int, user_id: int) -> bool:
        ...
        try:
            ...
            documents_in_category = self.db.query(Document).filter(
                and_(Document.category_id == category_id, Document.user_id == user_id)
            ).all()

            # Reassign to default category or uncategorized
            default_category = self.db.query(Category).filter(
                and_(Category.is_system_category == True, Category.name_en == "Archive")
            ).first()
            ...
            logger.info(f"Deleted category {category_id}, reassigned {len(documents_in_category)} documents")
            return True
    ...
    async def suggest_categories(self, text: str, user_id: int, limit: int = 5) -> List[Dict[str, Any]]:
        """Suggest categories based on document content"""
        try:
            # Get all categories available to user (from database)
            categories = await self.get_user_categories(user_id, include_system=True, include_user=True)
            ...
            for category in categories:
                ...
                if category["name_en"].lower() in text_lower or category["name_de"].lower() in text_lower:
                    score += 2
        ...
        try:
            ...
            for stat in formatted_stats:
                if total_documents > 0:
                    stat["percentage"] = (stat["document_count"] / total_documents) * 100
                else:
                    stat["percentage"] = 0
    ...
    async def search_categories(self, user_id: int, query: str, include_system: bool = True,
                              include_user: bool = True, language: str = "en") -> List[Dict[str, Any]]:
        """Search categories by name, description, or keywords"""
        try:
            if not query.strip():
                return await self.get_user_categories(user_id, include_system, include_user, language)
            ...
            for cat in categories:
                ...
                if query_lower in cat.name_en.lower() or query_lower in cat.name_de.lower():
                    score += 10
                if query_lower in (cat.description_en or "").lower() or query_lower in (cat.description_de or "").lower():
                    score += 5
