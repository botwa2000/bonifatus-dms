...
class CategoryService:
    ...
    async def create_category(self, category_data: Dict[str, Any]) -> Category:
        ...
        try:
            ...
            max_sort_order = (
                ...
                .filter(Category.user_id == category_data["user_id"])
                .count()
            )
    ...
    async def delete_category(
        self, category_id: int, user_id: int, move_documents_to: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                "documents_uncategorized": (
                    document_count if not move_documents_to else 0
                ),
            }
        ...
        try:
            ...
            for category in categories:
                score = self._calculate_category_relevance(
                    combined_text, category, language
                )
    ...
    async def get_category_statistics(self, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            uncategorized_count = (
                self.db.query(Document)
                .filter(
                    and_(Document.user_id == user_id, Document.category_id.is_(None))
                )
                .count()
            )
    ...
    async def reorder_categories(
        self, user_id: int, category_orders: List[Dict[str, int]]
    ) -> bool:
        """
    ...
    def _calculate_category_relevance(
        self, text: str, category: Category, language: str
    ) -> float:
        """
        ...
        try:
            if not category.keywords:
                ...
                fallback_keywords = [category.name_en.lower(), category.name_de.lower()]
