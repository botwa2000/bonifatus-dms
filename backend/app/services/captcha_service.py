...
class CaptchaService:
    ...
    def __init__(self):
        self._verification_cache: Dict[str, tuple] = {}
        self._cache_ttl = timedelta(minutes=5)

    @property
    def secret_key(self) -> Optional[str]:
        """Read secret key from settings (Docker Swarm secret)"""
        return settings.security.turnstile_secret_key or None

    def is_enabled(self) -> bool:
        """Check if Turnstile is configured — only the secret key is needed server-side"""
