...
class TestDocumentProcessing:
    ...
    async def test_document_upload_and_processing(self, client, auth_headers, test_user, test_db_session):
    ...
    async def test_keyword_extraction(self, test_db_session):
        ...
        assert any(word in ["financial", "invoice", "payment", "contract"] for word in keywords)
    ...
    async def test_document_list_with_filters(self, client, auth_headers, test_user):
        ...
        with patch('src.api.documents.AuthService') as mock_auth_class, \
            ...
            response = client.get("/api/v1/documents/?search=test", headers=auth_headers)
...
class TestCategorization:
    ...
    async def test_category_creation_api(self, client, auth_headers, test_user):
        ...
        with patch('src.api.categories.AuthService') as mock_auth_class, \
            ...
            mock_cat_service.initialize_default_categories = AsyncMock(return_value=True)
            mock_cat_service.create_user_category = AsyncMock(return_value=mock_category)
...
class TestSearch:
    ...
    async def test_document_search_api(self, client, auth_headers, test_user):
        ...
        with patch('src.api.search.AuthService') as mock_auth_class, \
             patch('src.api.search.SearchService') as mock_search_class:
            ...
            response = client.post("/api/v1/search/documents", json=search_data, headers=auth_headers)
    ...
    async def test_search_service_functionality(self, test_db_session, test_user):
        ...
        search_terms = search_service._parse_search_query('invoice "payment terms" contract')
    ...
    async def test_category_search_api(self, client, auth_headers, test_user):
        ...
        with patch('src.api.search.AuthService') as mock_auth_class, \
            ...
            mock_search_service = Mock()
            mock_search_class.return_value = mock_search_service
            mock_search_service.search_categories = AsyncMock(return_value={
                "categories": [
                    {
                        "id": 1,
                        "name": "Finance",
            ...
            response = client.get("/api/v1/search/categories?q=finance", headers=auth_headers)
    ...
    async def test_global_search_api(self, client, auth_headers, test_user):
        ...
        with patch('src.api.search.AuthService') as mock_auth_class, \
            ...
            mock_search_service = Mock()
            mock_search_class.return_value = mock_search_service
            mock_search_service.global_search = AsyncMock(return_value={
                "results": {
                    "documents": {
            ...
            response = client.post("/api/v1/search/global", json=search_data, headers=auth_headers)
...
class TestFullWorkflow:
    ...
    async def test_complete_document_workflow(self, client, auth_headers, test_user):
        ...
        with patch('src.api.documents.AuthService') as mock_auth_class, \
             patch('src.api.categories.AuthService') as mock_cat_auth_class, \
             patch('src.api.search.AuthService') as mock_search_auth_class:
            ...
            with patch('src.api.documents.DocumentService') as mock_doc_class:
                mock_doc_service = Mock()
                mock_doc_class.return_value = mock_doc_service
                mock_doc_service.validate_upload = AsyncMock(return_value={"valid": True})
                
                mock_document = Mock()
                mock_document.id = 1
                mock_document.filename = "test.pdf"
                mock_document.status = DocumentStatus.UPLOADING
                mock_doc_service.create_document_record = AsyncMock(return_value=mock_document)
            ...
            with patch('src.api.categories.CategoryService') as mock_cat_class:
                ...
                mock_cat_class.return_value = mock_cat_service
                mock_cat_service.initialize_default_categories = AsyncMock(return_value=True)
                mock_cat_service.get_user_categories = AsyncMock(return_value=[
                    {
                        "id": 1,
                        "name": "Finance",
                        "name_en": "Finance",
